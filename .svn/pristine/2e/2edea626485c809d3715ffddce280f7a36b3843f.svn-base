...
namespace GEO_DROID.Services.SincroService
{
    public class DBHelper
    {
        ...
        public static void ReMapForeignKeys<T>(GeoDroidDatabase _dbContext) where T : class, new()
        {
            //Type type = typeof(T);
        ...
        public static T GetValorConfigurationGEO<T>(string codigo, GeoDroidDatabase _dbContext, T defaultValue)
        {
            try
            {
                var config = _dbContext._database.Table<ConfigurationGEO>().ToListAsync().Result
        ...
        public static List<CacheImportes> GetCacheImportes(GeoDroidDatabase _dbContext, DateTime fechaDesde, DateTime fechaHasta)
        {
            List<CacheImportes> list = new List<CacheImportes>();

            var cacheImportes = _dbContext._database.Table<CacheImportes>()
        .Where(c => c.fecha >= fechaDesde && c.fecha <= fechaHasta)
        .ToListAsync().Result;
            var v = cacheImportes.Select(c => new
            {
                Cache = c,
                CacheConceptos = _dbContext._database.Table<CacheImportesConceptosRecaudacion>().ToListAsync().Result
                                             .Where(cc => cc.idCacheImportes == c.id)
                                             .ToList()
            }).ToList();
        ...
        public static List<CacheImportes> GetResumenImportes(GeoDroidDatabase _dbContext, bool cargarCache, DateTime fechaDesdeCache)
        {
            List<CacheImportes> list = new List<CacheImportes>();
            ...
            // ---- Datos actuales ----
            CacheImportes cache = new CacheImportes();//12/13/14/15

            DateTime fechaDesde = DateTime.MinValue;
            CacheImportes ultimoCache = _dbContext._database.Table<CacheImportes>().ToListAsync().Result.OrderByDescending(x => x.fecha).FirstOrDefault();
            if (ultimoCache != null)
                fechaDesde = ultimoCache.fecha;

            // RECAUDACIONES 
            var lRec = from r in _dbContext._database.Table<Recaudacion>().ToListAsync().Result.AsQueryable()
                       join inci in _dbContext._database.Table<Incidencia>().ToListAsync().Result.AsQueryable() on r.idIncidencia equals inci.id
            ...
            // CARGAS
            decimal cargas = (from c in _dbContext._database.Table<Carga>().ToListAsync().Result.AsQueryable()
                              join inci in _dbContext._database.Table<Incidencia>().ToListAsync().Result.AsQueryable() on c.idIncidencia equals inci.id
                              where c.idGeo == 0 && inci.fecha > fechaDesde
                              select c.cargaEmpresa).Sum();
            cache.cargas = cargas;

            // Recuperaciones de cargas
            decimal recuperacionesCargas = (from c in _dbContext._database.Table<Carga>().ToListAsync().Result.AsQueryable()
                                            join inci in _dbContext._database.Table<Incidencia>().ToListAsync().Result.AsQueryable() on c.idIncidencia equals inci.id
                                            where c.idGeo == 0 && inci.fecha > fechaDesde
            ...
            // GASTOS 
            decimal gastos = (from g in _dbContext._database.Table<GastoEstablecimiento>().ToListAsync().Result.AsQueryable()
                              where g.fecha > fechaDesde
                              select g.total).Sum();
            cache.gastos = gastos;

            // PRÉSTAMOS
            decimal prestamos = (from p in _dbContext._database.Table<Prestamo>().ToListAsync().Result.AsQueryable()
                                 where p.idGeo == 0 && p.fecha > fechaDesde
                                 select p.importe).Sum();
            cache.prestamos = prestamos;

            // Recuperaciones de préstamos
            decimal recuperacionesPrestamos = (from p in _dbContext._database.Table<PrestamoRecuperacion>().ToListAsync().Result.AsQueryable()
