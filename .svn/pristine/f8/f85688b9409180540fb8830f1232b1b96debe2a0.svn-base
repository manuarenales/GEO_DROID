        ...
        public async Task CreateAveriaFromState(CreateAveriaFromState action, IDispatcher dispatcher)
        {
            try
            {
                ...
                if (_averiaFormState.Value.AveriaSelected.id is not 0)
                {
                    ...
                    if (averia.AveriaEstado.fin)
                    {
                        averia.fechaModificacion = DateTime.Now;
                    }
                    else
                    {
                        averia.fechaModificacion = DateTime.MinValue;
                    }
                ...
                else
                {
                    ...
                    _averiaFormState.Value.AveriaSelected.idIncidencias = Inci.id;

                    _averiaFormState.Value.AveriaSelected.idAveriasEstados = _averiaFormState.Value.AveriaSelected.AveriaEstado.id;
                    _averiaFormState.Value.AveriaSelected.idConceptoAverias = _averiaFormState.Value.AveriaSelected.ConceptoAveria.id;

                    // estado marca fin si es true le meto fecha de finalizacion 
                    if (_averiaFormState.Value.AveriaSelected.AveriaEstado.fin)
                    {
                        _averiaFormState.Value.AveriaSelected.fechaModificacion = DateTime.Now;
                    }
                    else
                    {
                        _averiaFormState.Value.AveriaSelected.fechaModificacion = DateTime.MinValue;
                    }

                    int done = await _database._database.InsertAsync(_averiaFormState.Value.AveriaSelected);

                    //Carga 
                    if (_averiaFormState.Value.CargaSelected is not null)
                    {
                        GeoDroid.Data.Carga carga = _averiaFormState.Value.CargaSelected;
                        carga.idIncidencia = _averiaFormState.Value.AveriaSelected.idIncidencias;
                        carga.idAverias = _averiaFormState.Value.AveriaSelected.id;
                    ...
                    if (_averiaFormState.Value.LecturaContador is not null)
                    {
                        ...
                        if (lectura.id is not 0)
                        {
                            lectura.idIncidencias = _averiaFormState.Value.AveriaSelected.idIncidencias;
